...
class BuscarScreen2 extends StatefulWidget {
   
  const BuscarScreen2({Key? key}) : super(key: key);

  @override
  State<BuscarScreen2> createState() => _BuscarScreenState2();
}

class _BuscarScreenState2 extends State<BuscarScreen2> {
                ...
                onChanged: (value) async {
                  ...
                  if(_debounce?.isActive ?? false) _debounce?.cancel();
                      ...
                      setState(() {
                         ...
                         print('BUSQUEDA: ${plantsService.matchPlants.length}'); 
                      });
                   });
                 ...
                 CustomCategoryList(size : size, matchPlants: matchPlants, plants: plants) 
              ],
            ),
          ),
        ),
        // plants.isEmpty ? CustomIconEmptyPage(marginTop: 90, 
        //     onPressed: () async{
        //     await plantsService.loadPlants();
        // }) :
        SliverGrid(
          delegate: SliverChildBuilderDelegate((context, index) {
            return GestureDetector(
          onTap: (){
              plantsService.selectedPlant = plants[index].copy(false);
              Navigator.pushNamed(context, 'planta');
           },
          child: SquareCardTest(
              size: size, 
              img: plants[index].imagen ?? 'https://uning.es/wp-content/uploads/2016/08/ef3-placeholder-image.jpg', 
              estandar: plants[index].nombreComun ?? '',
              cientifico: plants[index].nombreCientifico ?? ''
            ),
          );
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            mainAxisSpacing: 3,
            crossAxisSpacing: 3,
            childAspectRatio: ((size.width*0.4)/2)/((size.height*0.23)/2),
            )
          )
      ],
    ),
